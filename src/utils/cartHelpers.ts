...
export const getDesignSafely = (
  ...
  if ("design" in item && item.design) {
    ...
    if (
      "back" in item.design &&
      "left" in item.design &&
      "right" in item.design &&
      "status" in item.design
    ) {
      item.design.quantity = item.quantity;
 ...
 */
export const getImageUrlFromItem = (
    ...
    if (images.length > 0) {
      ...
      if (images.length >= 2) {
        ...
        switch (view) {
          ...
          case "left":
            return images.length > 2 ? images[2] : images[0];
          case "right":
            return images.length > 3 ? images[3] : images[0];
          default:
            return images[0];
        }
      }
...
export const getItemPriceSafely = (item: CartItem | OrderItem): number => {
  ...
  if (
    "design" in item &&
    item.design &&
    "price" in item.design &&
    typeof item.design.price === "number"
  ) {
