...
const CustomizeHTMLV2 = () => {
  ...
  const getViewButtonClass = (isSelected: boolean) => {
    if (isSelected) {
      return 'px-3 py-1 text-sm rounded-full bg-blue-600 text-white';
    }
    return 'px-3 py-1 text-sm rounded-full bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200';
  };
              ...
              <Card className="p-6 border rounded-xl shadow-lg">
                <div className="rounded-xl overflow-hidden mb-6">
                  ...
                  <div ref={previewRef} className="relative aspect-[3/4] w-full max-w-xl mx-auto mb-6 rounded-lg overflow-hidden shadow-lg border border-gray-200">
                    ...
                    {(formValues.text || selectedImage) && (
                      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-white/80 dark:bg-gray-800/80 p-2 rounded-md text-center">
                        <span className="text-sm">
                          {selectedImage ? 'Arrastra las esquinas para redimensionar la imagen' : 'Arrastra para mover el texto'}
                        </span>
                      </div>
                    )}
                  </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {fontOptions.map((option) => (
                            <button
                              key={option.id}
                              type="button"
                              className={getFontButtonClass(formValues.font === option.id)}
...
export default CustomizeHTMLV2;
