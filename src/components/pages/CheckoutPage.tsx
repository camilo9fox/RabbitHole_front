...
import { convertCustomDesignToDTO, useCart } from "@/context/CartContext";
...
const CheckoutPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log("Cart item types available:", {
      standard: CartItemType.STANDARD,
  ...
  const createPersonalizedProductAndGetOrderItems = async () => {
    ...
    if (!hasCustomItems) return [];
    try {
      const response = await Promise.all(
        personalizedDesigns
          .filter((design) => design !== null)
          .map(async (design) => {
            const personalizedDesign = await createPersonalizedDesign(design);
            return {
              ...personalizedDesign,
              cantidad: design.cantidad,
              // angulos: design.angulos,
            };
          })
      );
      return response.map((design) =>
        convertPersonalizedDesignToOrderItem(design)
      );
    } catch (error) {
  ...
  const convertPersonalizedDesignToOrderItem = (
    design: DisenoPersonalizadoDTO
  ) => {
    return {
      disenoPersonalizadoId: design.id,
      tipoItemId: 2,
      cantidad: design?.cantidad ?? 1,
      colorId: design.colorId,
      tallaId: design.tallaId,
      // angulos: design.angulos,
    };
  };
  ...
  // const addThumbnailToCustomOrderItem = async (orderItems: any[]) => {
  //   await Promise.all(
  //     orderItems.map(async (item) => {
  //       item.angulos.map(async (angle: any) => {
  //         await addThumbnailOrderItem(
  //           item.id,
  //           angle.nombreAngulo,
  //           angle.thumbnailBase64
  //         );
  //       });
  //     })
  //   );
  ...
  const addThumbnailToProductOrderItem = async (orderItems: any[]) => {
    await Promise.all(
      orderItems.map(async (item) => {
        await addThumbnailOrderItem(
          item.id,
          "frente",
          item.previewImages.frente
        );
        await addThumbnailOrderItem(
          item.id,
          "espalda",
          item.previewImages.espalda
        );
        await addThumbnailOrderItem(
          item.id,
          "izquierda",
          item.previewImages.izquierda
        );
        await addThumbnailOrderItem(
          item.id,
          "derecha",
          item.previewImages.derecha
        );
      })
    );
  };
      ...
      >
        {cart.items.map((item, index) => {
          ...
          return (
            <li
              key={itemKey}
              className="py-4 flex overflow-hidden cart-item-container w-full"
            >
              <div className="flex">
                <div
                    ...
                    getDesignViewsSafely(design).map((viewKey: string) => {
                      ...
                      return (
                        <button
                          ...
                          className={`my-0.5 px-1.5 py-0.5 text-xs rounded-md ${
                            isActive
              ...
              <div className="ml-4 flex flex-1 flex-col overflow-hidden">
                <div
                  ...
                  } cart-item-title min-w-0`}
                >
                  <h3 className="truncate pr-2 max-w-[60%]">{name}</h3>
                  <p className="ml-2 flex-shrink-0 text-right">
                    {formatPrice(price)}
                  </p>
                </div>
                <p
                ...
                <div className="flex items-end justify-between text-sm w-full mt-1">
                  ...
                  <button
                    type="button"
                    className={`font-medium whitespace-nowrap flex-shrink-0 ml-2 px-2 py-0.5 rounded ${
                      isDarkMode
  ...
  const renderCartSummary = () => (
    <div
      className={`rounded-lg p-4 shadow-md cart-summary-container ${
        isDarkMode
    ...
    >
      ...
      >
        ...
        <div className="flex justify-between py-2 text-sm">
          ...
          <p className={`${isDarkMode ? "text-white" : "text-gray-900"}`}>
            {cart.totalPrice > 0 ? "Calculado en el siguiente paso" : "$0"}
          </p>
        </div>
        <div className="flex justify-between py-2 text-sm">
          <p className={`${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>
      ...
      {hasPendingDesigns && (
        ...
        >
          ...
          >
            Tu pedido contiene diseños personalizados pendientes de aprobación.
            Estos serán revisados antes de procesar tu compra.
          </p>
        </div>
      )}
    </div>
  ...
  const renderConfirmation = () => (
    <div className="space-y-6 text-center">
      ...
      <p className={`${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
        Hemos recibido tu pedido y lo estamos procesando. Recibirás un correo
      ...
      {hasPendingDesigns && (
        <div
          className={`mt-4 p-4 border rounded-md text-left ${
            isDarkMode
        ...
        >
          ...
          >
            Tu pedido contiene diseños personalizados que necesitan ser
            aprobados. Te notificaremos por correo electrónico cuando sean
            revisados.
          </p>
        </div>
      )}
