...
const ProductCanvas = forwardRef<ProductCanvasRefHandle, ProductCanvasProps>(
  ({ angle, color, className = "", design }, ref) => {
    ...
    const [processedDesignImage, setProcessedDesignImage] = useState<
      DesignImage | undefined
    >(undefined);
    const [processedDesignText, setProcessedDesignText] = useState<
    ...
    useLayoutEffect(() => {
      console.log(
        `ProductCanvas: Procesando diseño para ángulo ${angle}:`,
        design
      );
      ...
      if (design.text) {
        ...
        console.log(
          `ProductCanvas: Texto procesado para ángulo ${angle}:`,
          processedText
        );
      ...
      if (design.image) {
        ...
        console.log(
          `ProductCanvas: Imagen procesada para ángulo ${angle}:`,
          processedImage
        );
        ...
        preloadImage(processedImage.src)
          .then(() => {
            console.log(
              `ProductCanvas: Imagen precargada correctamente para ángulo ${angle}`
            );
          })
          .catch((error) => {
            console.error(
              `Error al precargar la imagen para ángulo ${angle}:`,
              error
            );
          });
      } else {
        ...
        >
          <div className="w-full h-full flex items-center justify-center p-4">
            ...
            <HTMLCanvasV2
              ...
              key={`${angle}-${color}-${
                processedDesignImage?.src ?? "no-image"
              }`} // Forzar recreación al cambiar imagen o ángulo
            />
