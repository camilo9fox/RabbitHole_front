...
const renderDesktopAuthenticatedContent = (
...
const renderMobileAuthenticatedContent = (
  session: Session | null,
  isDarkMode: boolean
) => {
  return (
...
const renderUserMenu = (session: Session | null, isDarkMode: boolean) => {
  return (
    <div
      className={`absolute top-10 right-0 w-56 rounded-md shadow-lg py-1 z-50 ${
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        ...
        onClick={() =>
          signIn("azure-ad", { callbackUrl: "/home", redirect: true })
        }
      >
    ...
    if (typeof window !== "undefined") {
      if (session?.accessToken) {
        try {
          ...
          document.cookie =
            "authToken=" + session.accessToken + "; path=/; max-age=2592000"; // 30 días
    ...
    <div className="flex items-center gap-2" suppressHydrationWarning>
      {isAuthenticated ? (
        <>
          ...
          {renderDesktopAuthenticatedContent(
            session,
            isDarkMode,
            menuOpen,
            setMenuOpen,
