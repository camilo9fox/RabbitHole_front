...
const HTMLCanvasV2 = forwardRef<HTMLCanvasHandle, HTMLCanvasProps>(
  (
    ...
    const drawCanvas = useCallback(() => {
      ...
      console.log(
        "Dibujando canvas con imagen:",
        tshirtImg ? "cargada" : "no cargada"
      );
      ...
      (e: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        if (isDragging && dragTarget && canvasRef.current) {
          if (dragTarget === "image" && userImg) {
            ...
            newY = Math.max(
              halfHeight,
              Math.min(newY, canvasHeight - halfHeight)
            );
            ...
            if (ctx) {
              ...
              newX = Math.max(
                textWidth / 2 + 5,
                Math.min(newX, canvasWidth - textWidth / 2 - 5)
              );
              newY = Math.max(
                textHeight / 2 + 5,
                Math.min(newY, canvasHeight - textHeight / 2 - 5)
              );
            }
        ...
        if (isResizing && resizeHandle && userImg) {
          ...
          if (newPosX + halfNewWidth > canvasWidth)
            newPosX = canvasWidth - halfNewWidth;
          if (newPosY - halfNewHeight < 0) newPosY = halfNewHeight;
          if (newPosY + halfNewHeight > canvasHeight)
            newPosY = canvasHeight - halfNewHeight;
