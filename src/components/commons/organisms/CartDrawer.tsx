...
const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      // Animar apertura
      tl.to(overlay, { opacity: 0.5, duration: 0.2 }).to(
        drawer,
  ...
  const getCustomizedViews = (
    ...
    views.forEach((view) => {
      ...
      const hasPreviewImage =
        viewData.previewImage !== undefined && viewData.previewImage !== null;
  ...
  const getCustomItemDetails = (design: CustomDesign): string => {
    let details = `${
      colors.find((c) => c.id === design.color)?.label ?? "Sin color"
    } / Talla ${design?.size?.toUpperCase() ?? "N/A"}`;
  ...
  const renderCartItem = (item: CartItem, index: number) => {
            ...
            <div>
              ...
              >
                {product
                  ? `${
                      colors.length > 0
                        ? colors.find((c) => c.id === product.color)?.label
                        : ""
                    } / Talla ${product.size?.toUpperCase() || "N/A"}`
    ...
    if (isCustomItem(item)) {
        ...
        return (
          <li key={getCartItemKey(item, index)} className="py-6">
            <p className={`${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
              Diseño sin vistas personalizadas
            </p>
          </li>
        );
      }
      ...
      const activeView =
        activeViews[itemKey] || customizedViews[0]?.id || "front";

      // Encontrar la vista activa en las vistas personalizadas
      const currentView =
        customizedViews.find((view) => view.id === activeView) ||
        customizedViews[0];
